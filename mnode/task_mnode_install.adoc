...
.Pasos
. Compruebe que la hora se sincronice entre el nodo de gestión y el clúster de almacenamiento mediante NTP:
...
.. Añade una nueva línea por cada servidor de hora predeterminado que quieras añadir. Los servidores de hora predeterminados deben ser los mismos servidores NTP que se usan en el clúster de almacenamiento que vas a usar en un <<set-up-the-management-node,paso posterior>>.
+
[listing]
----
vi /etc/ntp.conf
...
*** *--storage_mvip (-sm) [dirección MVIP]*: La dirección IP virtual de gestión (MVIP) del clúster de almacenamiento en el que se ejecuta el software Element. Configura el nodo de gestión con el mismo clúster de almacenamiento que usaste durante link:task_mnode_recover.html#configure-time-sync["Configuración de servidores NTP"].
