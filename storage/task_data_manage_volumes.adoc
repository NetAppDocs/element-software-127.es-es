...
NOTE: Los sistemas operativos difieren en cómo tratan los volúmenes clonados. VMware ESXi tratará un volumen clonado como una copia de volumen o un volumen de snapshot. El volumen será un dispositivo disponible para usar y crear un nuevo datastore. Para más información sobre cómo montar volúmenes clonados y manejar LUN de snapshot, consulta la documentación de VMware en https://techdocs.broadcom.com/["Montar una copia de almacén de datos VMFS"^] y https://techdocs.broadcom.com/["Gestión de almacenes de datos VMFS duplicados"^].
