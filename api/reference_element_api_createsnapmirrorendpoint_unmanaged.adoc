...
[role="lead"]
El sistema de almacenamiento del software Element usa `CreateSnapMirrorEndpointUnmanaged` el método para permitir que los extremos remotos y no gestionados de SnapMirror se comuniquen con un clúster de almacenamiento Element. No se pueden administrar extremos no gestionados mediante las API de SnapMirror de Element. Deben gestionarse con las API o el software de gestión de ONTAP.
