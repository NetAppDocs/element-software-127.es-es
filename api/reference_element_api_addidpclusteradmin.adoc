...
[role="lead"]
Puede usar `AddIpdClusterAdmin` el método para añadir un usuario administrador de clúster autenticado por un proveedor de identidades (IdP) de terceros. Las cuentas de administrador del clúster de IDP se configuran según la información de valor de atributos de SAML que se proporciona dentro de la afirmación de SAML del IDP asociada con el usuario. Si un usuario se autentica correctamente con el IDP y tiene declaraciones de atributos SAML dentro de las afirmaciones de SAML que coinciden con varias cuentas de administrador de clúster IDP, el usuario tendrá el nivel de acceso combinado de las cuentas de administrador de clúster de IDP que coincidan.
...
nombre de usuario
 a| 
Asignación de atributo-valor de SAML a un administrador del clúster IDP (por ejemplo, email=test@example.com). Esto se puede definir mediante un asunto SAML específico utilizando `NameID` o como una entrada en la sentencia de atributo SAML, como `eduPersonAffiliation` .
 a| 
cadena
 a| 
Ninguno
