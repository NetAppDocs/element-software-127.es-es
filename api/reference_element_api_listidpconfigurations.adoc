...
[role="lead"]
Puede utilizar el `ListIdpConfigurations` método para enumerar configuraciones de IDPs de terceros. De manera opcional, se puede proporcionar `enabledOnly` la marca para recuperar la configuración de IdP habilitada actualmente o un UUID de metadatos de IdP o un nombre de IdP para consultar información sobre una configuración de IdP específica.
