...
[role="lead"]
Puede usar `EnableSsh` el método para habilitar el servicio Secure Shell (SSH) en un solo nodo. Este método no afecta a la duración del tiempo de espera de SSH para todo el clúster y no exime al nodo de que SSH esté deshabilitado por el tiempo de espera global de SSH.
