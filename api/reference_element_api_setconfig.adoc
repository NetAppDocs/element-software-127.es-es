...
[role="lead"]
Puede usar el `SetConfig` método para establecer la información de red y de clúster para el nodo. Este método incluye la misma configuración en un solo método API que está disponible mediante ambos `SetClusterConfig` métodos y. `SetNetworkConfig` Sólo es necesario incluir con este método los campos que desee modificar.
