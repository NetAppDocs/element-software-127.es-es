...
[role="lead"]
Se puede utilizar `CreateSchedule` para programar una snapshot automática de un volumen en un intervalo definido.
...
* `Offset`: Para cada semana posible en un mes, 1 a 6 (Si es mayor que 1, solo coincide el día Nth-1 de la semana. Por ejemplo, offset:3 para domingo significa el tercer domingo del mes, mientras que offset:4 para miércoles significa el cuarto miércoles del mes. Offset:0 significa que no se realiza ninguna acción. Offset:1 (predeterminado) significa que la snapshot se crea para este día de la semana, independientemente de dónde esté en el mes
