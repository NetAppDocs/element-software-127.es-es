...
[role="lead"]
El objeto pendingNode contiene información sobre un nodo que se puede añadir a un clúster. Utilice el `ListPendingNodes` método API para obtener una lista con esta información de todos los nodos pendientes. Puede añadir cualquiera de los nodos mostrados a un clúster mediante el `AddNodes` método API.
