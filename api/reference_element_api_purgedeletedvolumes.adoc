...
[role="lead"]
Puede usar el `PurgeDeletedVolumes` método para purgar de forma inmediata y permanente volúmenes que se eliminaron; puede usar este método para purgar hasta 500 volúmenes a la vez.

Es necesario eliminar los volúmenes `DeleteVolumes` para poder purgarse. Los volúmenes se purgan automáticamente después de un período de tiempo, por lo que normalmente no se requiere el uso de este método.
