...
[role="lead"]
Puede usar el `GetLimits` método para obtener los valores límite establecidos por la API. Estos valores pueden cambiar entre las versiones de elemento, pero no cambian sin una actualización del sistema. Conocer los valores de límite establecidos por la API puede ser útil al escribir scripts de API para herramientas orientadas al usuario.


NOTE: El `GetLimits` método devuelve los límites de la versión de software actual independientemente de la versión de punto final de API utilizada para pasar el método.
