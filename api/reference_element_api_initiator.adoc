...
[role="lead"]
El objeto iniciador contiene información acerca de un iniciador de iSCSI o Fibre Channel. Un objeto iniciador puede contener identificadores de IQN o WWPN. Puede usar el `ListInitiators` método para obtener una lista de todos los iniciadores conocidos en el sistema. Los objetos iniciadores se utilizan para configurar el acceso del iniciador SCSI a un conjunto de volúmenes mediante grupos de acceso de volúmenes. Un iniciador solo puede ser miembro de un grupo de acceso de volúmenes a la vez. Puede restringir el acceso del iniciador a una o varias VLAN si especifica una o varias virtualNetworkIDs mediante los `CreateInitiators` métodos y. `ModifyInitiators` Si no especifica ninguna red virtual, el iniciador puede acceder a todas las redes.
