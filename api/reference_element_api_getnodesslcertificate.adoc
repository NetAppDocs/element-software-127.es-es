---
permalink: api/reference_element_api_getnodesslcertificate.html 
sidebar: sidebar 
keywords: ssl,cert,certificate,get,getting,getnodesslcertificate,active 
summary: Puede usar el método GetNodeSSLCertificate para recuperar el certificado SSL que está activo en ese momento en el nodo de gestión. 
---
= GetNodeSSLCertificate
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Es posible usar el `GetNodeSSLCertificate` método para recuperar el certificado SSL que actualmente está activo en el nodo de gestión.



== Parámetros


NOTE: Debe llamar a este método contra el nodo de gestión. Por ejemplo:

[listing]
----
https://<management node IP>:442/json-rpc/10.0
----
Este método no tiene parámetros de entrada.



== Valores devueltos

Este método tiene los siguientes valores devueltos:

|===
| Nombre | Descripción | Tipo 


 a| 
certificado
 a| 
El texto completo codificado con PEM del certificado.
 a| 
cadena



 a| 
detalles
 a| 
La información descodificada del certificado.
 a| 
Objeto JSON

|===


== Ejemplo de solicitud

Las solicitudes de este método son similares al ejemplo siguiente:

[listing]
----
{
    "method" : "GetNodeSSLCertificate",
    "params" : {},
    "id" : 1
}
----


== Ejemplo de respuesta

Este método devuelve una respuesta similar al siguiente ejemplo:

[listing]
----
{
    "id": 1,
    "result": {
        "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n",
        "details": {
            "issuer": "/C=US/ST=NV/L=Denver/O=NetApp/emailAddress=test@netapptest.org",
            "modulus": "",
            "notAfter": "2027-03-06T22:50:26Z",
            "notBefore": "2017-03-08T22:50:26Z",
            "serial": "CC1B221598E37FF3",
            "sha1Fingerprint": "1D:70:7A:6F:18:8A:CD:29:50:C7:95:B1:DD:5E:63:21:F4:FA:6E:21",
            "subject": "/C=US/ST=NV/L=Denver/O=NetApp/emailAddress=test@netapptest.org"
        }
    }
}
----