...
[role="lead"]
El software Element utiliza esta versión `ModifySnapMirrorEndpoint` del método para modificar los atributos de nombre o dirección IP del clúster de almacenamiento para un extremo SnapMirror no gestionado. No se pueden administrar extremos no gestionados mediante las API de SnapMirror de Element. Deben gestionarse con las API o el software de gestión de ONTAP.
