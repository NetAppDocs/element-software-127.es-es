...
[role="lead"]
El objeto ldapConfiguration contiene información sobre la configuración de LDAP en el sistema de almacenamiento de. Puede recuperar información de LDAP con el `GetLdapConfiguration` método API.
...
URIs de servidor
 a| 
Lista separada por comas de URI de servidor LDAP (por ejemplo, `ldap://1.2.3.4` y `ldaps://1.2.3.4:123`.)
 a| 
cadena
