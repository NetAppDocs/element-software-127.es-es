...
[role="lead"]
Puede usar el `CreateVolume` método para crear un nuevo volumen vacío en el clúster. Apenas se crea el volumen, el volumen está disponible para la conexión a través de iSCSI.

Los volúmenes creados sin valores de calidad de servicio especificados utilizan los valores predeterminados. Para ver los valores predeterminados de un volumen, utilice `GetDefaultQoS` el método.
...
| `access` | El modo de acceso del volumen. Si se incluye este parámetro, el único valor soportado es `snapMirrorTarget`. | cadena | Ninguno | No 
...
* `false`: la emulación 512e no está activada.
