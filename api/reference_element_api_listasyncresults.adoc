...
[role="lead"]
Puede utilizar `ListAsyncResults` para enumerar los resultados de todos los métodos asíncronos actualmente en ejecución y completados en el sistema. La consulta de resultados asíncronos con `ListAsyncResults` no causa que caduquen las Handles de Procurary completadas; puede utilizar `GetAsyncResult` para consultar cualquiera de las Handles de Procurary devueltas por `ListAsyncResults`.
