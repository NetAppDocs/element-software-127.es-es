...
[role="lead"]
El objeto de la unidad contiene información sobre las unidades individuales de los nodos activos del clúster. Este objeto contiene detalles sobre las unidades que se añadieron como metadatos de volúmenes o unidades de bloques, así como las unidades que todavía no se han añadido y están disponibles. Puede recuperar esta información con el `ListDrives` método API.
