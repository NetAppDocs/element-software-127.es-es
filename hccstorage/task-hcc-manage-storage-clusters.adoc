...
[role="lead"]
Puede añadir clústeres de almacenamiento al inventario de activos del nodo de gestión para poder gestionarlos mediante el control del cloud híbrido (HCC) de NetApp. El primer clúster de almacenamiento que se agregó durante la configuración del sistema es el valor predeterminadolink:../concepts/concept_intro_clusters.html#authoritative-storage-clusters["clúster de almacenamiento fiable"], pero se pueden añadir clústeres adicionales mediante la interfaz de usuario de HCC.
...
Si necesita desconectar un nodo de almacenamiento para realizar tareas de mantenimiento, como actualizaciones de software o reparaciones de host, puede minimizar el impacto de I/O en el resto del clúster de almacenamiento mediante <<enable_main_mode,habilitación>>el modo de mantenimiento de ese nodo. Cuando <<disable_main_mode,desactivar>>el modo de mantenimiento, el nodo se supervisa para garantizar que se cumplen determinados criterios antes de que el nodo pueda salir del modo de mantenimiento.
...
[[enablemainmodeenable-maintenance-mode]]
...
[[disablemainmodedisable-maintenance-mode]]
